                                  ...
                                  ext:(NSString*)ext
{
    ...
    //方法1  直接生成filament texture（这个的纹理加载会延迟到command 执行）, 并设置进material instance的parameter
...
//    [self performSelector:@selector(setParameterOfMat) withObject:nil afterDelay:2];
    ...
    //方法2 用opengl加载纹理（这个的纹理加载马上执行），利用该纹理id生成filament texture然后设置进material instance的parameter
